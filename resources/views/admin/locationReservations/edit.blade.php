        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="m-2 p-2 bg-slate-100 rounded flex justify-center">
                <div class="space-y-8 divide-y divide-gray-200 w-full lg:w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.locationreservations.update', $reservation->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="first_name" name="name"
                                    value="{{ old('first_name', $reservation->user->name) }}"
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="tel_number" name="tel_number"
                                    value="{{ $reservation->user->tel_number }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('tel_number')
                                <div class="text-sm text-red-400">{{ $message }}</div>
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="datetime-local" id="res_date" name="checkout_date"
                                    value="{{ old('res_date', $reservation->checkout_date->format('Y-m-d\TH:i:s')) }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('checkout_date')
                                <div class="text-sm text-red-400">{{ $message }}</div>
